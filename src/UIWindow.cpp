...
void UIWindow::Init(){
    ...
    UIView::Init();
    rootWindow = this;// set to self so that when doing draw pass we draw from top down draw select pass needs pointer to root window(ourself)
    vpCntlr = new UIViewPortController(this, viewRect.size.width, viewRect.size.height, HORIZONTAL );
...
void UIWindow::connectNodes(int outputNode_id, int inputNode_id, int fromPlugID, int toPlugID ){
...
void UIWindow::InitGL(const char *name){
    ...
    //checkOpenGLError(); // still gets GL_INVALID_ENUM after glew though. can safely ingore.
    ...
    */
    ...
    frustrum.right = viewRect.size.width;;//+(viewRect.size.width/2.0);
    frustrum.bottom = 0.0;
...
void UIWindow::registerView(UIView *newView, UIView *sender){
        ...
        globalViewCount += 1;
        printf("no spares.increasing vector. view count is %d and vector size is %d\n",globalViewCount, (int)vUIViewGlobalStore.size());
        newView->setParentID(sender->globalIndexID);
        newView->Init();
...
void UIWindow::deRegisterView(int id) //delete a view.
{
        ...
        if ((vUIViewGlobalSpare[id-1] != NULL) && (id-1 < (int)vUIViewGlobalStore.size()) ) { //id less than vector size means it falls within range
                    ...
                    if (id == (int)vUIViewGlobalStore.size() ){ //if UIView id is at the end
                        ...
                        delete vUIViewGlobalStore[id-1];							//vNodeStore.erase(myIT+id-1);					//delete the element at the index specified by id
...
UIView* UIWindow::getNodeFromID(int id){
    ...
    return vUIViewGlobalStore.at(id);
    ...
    switch (key.mouseActionStore)
    {
        case GLFW_PRESS:// MOUSE BUTTON CLICKED
            ...
            switch (key.buttonStore)
            {
                ...
                {
                    lmbPressed = true;
                    cout << "mousepos in event handler is " << key.Mx << " _ " << key.My <<endl;
                    ...
                    cout << "in UIWindow .about to call node " << selectedViewID << " viewClicked" << endl;
